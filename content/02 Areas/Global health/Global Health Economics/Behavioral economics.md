---
tags:
  - Economics
  - Psychologie
publish: true
created: 2026-02-24T23:17
modified: 2026-02-25T22:47
---
# What is behavioral [[economics]]?

**Born out of the inherent paradoxes of neoclassical [[economics]]**
1. Why don’t economic actors align with the classic economic assumptions?
	1. Maximize utility
	2. Prefer mixed goods over extremes
	3. Have complete information
	4. Stable preferences
	5. Always want more
2. Why do economic actors act “irrational”?
	1. Systematic deviations from standard economic model (non-standard **preferences**, non-standard **beliefs**, non-standard **decision-making**)
	2. The **know-do gap**; 
		1. traditional biomedical research doesn’t account for human behavior
		2. health systems built on how things “should” work, not how they actually work in real life
		3. traditional behavior change methods (e.g., give information) can only go so far
	3. Design programs/interventions to avoid biases overcome some natural tendencies that lead to poor choices.

# Behavioral economic model
![[Pasted image 20260224232545.png]]
**HOMER ECONOMICUS**
- We may ignore costs or benefits
- We may misjudge which option we prefer most
- We use “rules of thumb” / intuition to make decisions
- We have limited ability to absorb and integrate information
- We may lack well-defined preferences

| Non-standard preferences                                        | Non-standard beliefs                                                  | Non-standard decision-making                                               |
| --------------------------------------------------------------- | --------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| - present-[[bias]]<br>- reference-dependence<br>- social preference | - naivete<br>- projection [[bias]]<br>- deviations from Bayesian learning | - limited attention and memory<br>- mental accounting<br>- default effects |
# Behavioral strategies

**How do we help people make better decisions?**
- Classic economic model
	- redistribute wealth
	- provide information
	- correct market failures (including **externalities**, costs our actions impose on others)
- Behavioral economic model
	- nudging
	- correct **internalities** (costs you impose on yourself)

## The MINDSPACE way

**M** essenger (who is talking matters)
**I** ncentives (increase the *immediate* benefits)
**N** orms (wanting to do what others are doing)
**D** efaults (go with the flow with preset options)
**S** alience (novel or relevant things get noticed)
**P** riming (reacting to subconscious cues (verbal, visual, auditory))
**A** ffect (acting according to how we feel, rather than how we think)
**C** ommitments (wanting to follow-through on our promises)
**E** go (act in ways that make us feel better about ourselves)